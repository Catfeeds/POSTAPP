...
function sumServerMoney()
{
	...
	if(!empty($_GPC['buyer_id_'.$appid]))
	{
	   ...
	   $TotalMoney1=pdo_fetchcolumn("select SUM(total_fee) from ".tablename("j_money_trade")." where weid=:weid and openid=:openid and createtime >:starttime and createtime<:endtime and status=1",array(
	    ":weid"=>$_W['uniacid'],
	    ":openid"=>$openid,
	    ":starttime"=>strtotime(date("Y-m-d")),
	    ":endtime"=>time(),
