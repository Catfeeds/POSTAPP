...
class Superman_creditmall_doWebTask extends Superman_creditmallModuleSite
{
	...
	public function exec()
	{
		...
		if ($act == 'display') {
			...
			superman_task_set($item);
			$types = superman_task_type();
			$groups = superman_mc_groups_fetchall(array('uniacid' => $_W['uniacid'],), '', 0, -1);
			$credits = superman_credit_type();
			if (checksubmit()) {
				$start_time = strtotime($_GPC['task_time']['start']);
				$end_time = strtotime($_GPC['task_time']['end']);
				$data = array('title' => trim($_GPC['title']), 'description' => trim($_GPC['description']), 'icon' => superman_fix_path(trim($_GPC['icon'])), 'credit_type' => trim($_GPC['credit_type']), 'credit_min' => floatval($_GPC['credit_min']), 'credit_max' => floatval($_GPC['credit_max']), 'starttime' => !isset($_GPC['time_limit']) ? $start_time : 0, 'endtime' => !isset($_GPC['time_limit']) ? $end_time : 0, 'isshow' => $_GPC['isshow'] ? 1 : 0, 'applied' => intval($_GPC['applied']), 'completed' => intval($_GPC['completed']), 'limits' => intval($_GPC['limits']), 'extend' => isset($_GPC['extend']) ? iserializer($_GPC['extend']) : '', 'displayorder' => intval($_GPC['displayorder']), 'showdata' => intval($_GPC['showdata']), 'applyperm' => $_GPC['applyperm'] ? iserializer($_GPC['applyperm']) : '',);
				if (!$item['builtin']) {
			...
			if (!$item['starttime'] && !$item['endtime']) {
				$item['task_time'] = array('start' => date('Y-m-d H:i'), 'end' => date('Y-m-d H:i', strtotime('+1 month')),);
			}
			$house_name = '';
			if ($item['name'] == 'superman_house' && isset($item['extend']['houseid'])) {
				$sql = "SELECT name FROM " . tablename('supermanfc_house') . " WHERE id=:id";
				$params = array(':id' => $item['extend']['houseid'],);
			...
			if ($item['name'] == 'superman_sign' && isset($item['extend']['rid'])) {
				...
				$params = array(':rid' => $item['extend']['rid'],);
			...
			$task_list = M::t('superman_creditmall_task')->fetchall(array('uniacid' => $_W['uniacid'],), '', 0, -1);
			$pindex = max(1, intval($_GPC['page']));
			$pagesize = 20;
			$start = ($pindex - 1) * $pagesize;
			$filter = array('uniacid' => $_W['uniacid'],);
			...
			if ($total > 0) {
				...
				if ($list) {
					$task_status_style = array('-1' => 'label label-danger', '0' => 'label label-default', '1' => 'label label-success',);
					foreach ($list as &$item) {
			...
			$log = array($_W['uid'], "【{$task['title']}】任务奖励", 'superman_creditmall',);
			$ret = mc_credit_update($mytask['uid'], $mytask['credit_type'], $mytask['credit'], $log);
			if (is_error($ret)) {
				message('更新积分失败！result=' . var_export($ret, true), '', 'error');
			}
			$data = array('status' => 1, 'completetime' => TIMESTAMP,);
			$condition = array('id' => $id,);
			M::t('superman_creditmall_mytask')->update($data, $condition);
			M::t('superman_creditmall_task')->increment(array('completed' => 1,), array('id' => $task['id'],));
			message('操作成功！', referer(), 'success');
		}
