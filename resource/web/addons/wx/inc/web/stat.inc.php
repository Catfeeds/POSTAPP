...
class Superman_creditmall_doWebStat extends Superman_creditmallModuleSite
{
	...
	private function stat_order($starttime, $endtime)
	{
		...
		for ($i = $starttime; $i <= $endtime; $i += (24 * 3600)) {
			...
			$filter = array('uniacid' => $_W['uniacid'], 'start_time' => $t1, 'end_time' => $t2,);
			$filter['status'] = 0;
	...
	private function stat_credit($starttime, $endtime)
	{
		...
		for ($i = $starttime; $i <= $endtime; $i += (24 * 3600)) {
			...
			foreach ($arr as $k => $credit) {
				$filter = array('uniacid' => $_W['uniacid'], 'credittype' => $credit['type'], 'start_time' => $t1, 'end_time' => $t2);
				if ($credit['sign'] > 0) {
					$filter['num_up'] = 0;
				} else {
					$filter['num_down'] = 0;
	...
	function _credit_stat_init()
	{
		...
		$all_colors = array('flow1' => '36,165,222', 'flow2' => '203,48,48', 'flow3' => '149,192,0', 'flow4' => '231,160,23', 'flow5' => '119,119,119', 'flow6' => '106,90,205', 'flow7' => '255,0,255', 'flow8' => '255,165,0', 'flow9' => '0,0,205', 'flow10' => '0,205,0',);
