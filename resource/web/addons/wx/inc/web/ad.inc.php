	...
	public function exec()
	{
		...
		if ($act == 'display') {
			...
			if ($id) {
				...
				$item = array('ad_time' => array('start' => date('Y-m-d H:i'), 'end' => date('Y-m-d H:i', strtotime('+1 month')),), 'isshow' => 1,);
			}
			if (checksubmit('submit')) {
				...
				if (isset($_GPC['content']['title']) && is_array($_GPC['content']['title'])) {
					foreach ($_GPC['content']['title'] as $k => $v) {
						$content[$k] = array('title' => $_GPC['content']['title'][$k], 'thumb' => $_GPC['content']['thumb'][$k], 'url' => $_GPC['content']['url'][$k],);
					}
					ksort($content);
				}
				$data = array('title' => trim($_GPC['title']), 'start_time' => !isset($_GPC['ad_time_limit']) ? $start_time : 0, 'end_time' => !isset($_GPC['ad_time_limit']) ? $end_time : 0, 'isshow' => $_GPC['isshow'] ? 1 : 0, 'displayorder' => intval($_GPC['displayorder']), 'content' => $content ? iserializer($content) : array(),);
				if ($id) {
				...
				if ($_GPC['position_ids'] && is_array($_GPC['position_ids'])) {
					foreach ($_GPC['position_ids'] as $posid) {
						$data = array('ad_id' => $id, 'position_id' => $posid,);
