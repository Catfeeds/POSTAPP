...
class Superman_creditmall_doWebDispatch extends Superman_creditmallModuleSite
{
	...
	public function exec()
	{
		...
		if ($act == 'display') {
			...
			if (checksubmit('submit')) {
				$data = array('uniacid' => $_W['uniacid'], 'displayorder' => $_GPC['displayorder'], 'title' => trim($_GPC['title']), 'fee' => $_GPC['fee'], 'isshow' => $_GPC['isshow'], 'need_address' => $_GPC['need_address'], 'extend' => iserializer($_GPC['extend']),);
				if (!$id) {
