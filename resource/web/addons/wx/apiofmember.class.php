...
class member
{
	...
	/*提交卡片加项目（赠送和购买都可以）
	 * 
	 * appid	Y	string	公众号appid
	 * cardId	Y	integer	卡片id
	 * itemIds	Y	string	项目id，以逗号相隔
	 * itemTimes	Y	string	对应项目的个数，以逗号相隔
	 * payAmount	  Y	decimal	                 购买金额，赠送填0
	 * billed	 Y	boolean	是否开发票
	 * expiredDate	   Y	string	到期时间（格式2018-03-15，-1代表没有到期日,添加多个项目以逗号分隔）    
	 * comments	 Y	string	备注内容，可以写活动内容   
	 * */
	public function saveService($data)
	{
		$type="save-card-service-set?";
		$type.=http_build_query($data);
		return $this->getCommon($type,true);
	}
	/*提交卡片加代金券（赠送和购买都可以）
