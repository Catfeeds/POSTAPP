...
{
	return array('_init' => 1, 'base' => array('wechat' => 1, 'debug' => 0, 'debug_message' => '系统升级中...', 'debug_uids' => array(1), 'creditrank' => array('credit1'),), 'wxpay' => array('mch_appid' => '', 'mchid' => '', 'sub_appid' => '', 'sub_mch_id' => ''), 'redpack' => array('help' => '',), 'help' => array('base' => '占位', 'use' => '占位', 'get' => '占位',), 'share' => array('title' => '你的积分价值超乎你的想象！', 'imgurl' => 'addons/superman_creditmall/template/mobile/images/share_img.jpg', 'desc' => '赶快抱走那堆发光的奖品吧！',), 'subscribe' => array('check' => 0, 'tips' => '',),);
}
...
function superman_setting_init($uniacid, $settings)
{
	...
	$params = array(':module' => 'superman_creditmall', ':uniacid' => $_W['uniacid'],);
	if (pdo_fetchcolumn($sql, $params)) {
		...
		pdo_insert('uni_account_modules', array('settings' => iserializer($settings), 'module' => 'superman_creditmall', 'uniacid' => $_W['uniacid'], 'enabled' => 1));
...
{
	$all_types = array('1' => array('id' => 1, 'name' => '一口价', 'title' => '热卖精品', 'enable' => 1, 'displayorder' => 50,), '2' => array('id' => 2, 'name' => '竞拍', 'title' => '夺宝奇兵', 'enable' => 1, 'displayorder' => 90,), '3' => array('id' => 3, 'name' => '猜价', 'title' => '我猜猜猜', 'enable' => 0, 'displayorder' => 70,), '4' => array('id' => 4, 'name' => '抽奖', 'title' => '幸运抽奖', 'enable' => 0, 'displayorder' => 60,), '5' => array('id' => 5, 'name' => '微信红包', 'title' => '抢红包', 'enable' => 1, 'displayorder' => 80,), '7' => array('id' => 7, 'name' => '会员专属', 'title' => '积分商品', 'enable' => 1, 'displayorder' => 100,),);
...
{
	return array('1' => '付款减库存', '2' => '拍下减库存', '3' => '永不减库存',);
}
...
function superman_product_set(&$product)
{
	...
	$product['activity_time'] = array('start' => $product['start_time'] ? date('Y-m-d H:i:s', $product['start_time']) : '', 'end' => $product['end_time'] ? date('Y-m-d H:i:s', $product['end_time']) : '',);
	$product['allow_sale'] = 0;
	if ($product['total'] > 0 && (($product['start_time'] == 0 && $product['end_time'] == 0) || ($product['start_time'] > 0 && $product['start_time'] <= TIMESTAMP && $product['end_time'] > 0 && $product['end_time'] >= TIMESTAMP))) {
...
function superman_product_update_sales($id, $total = 1)
{
	...
	if ($product) {
		...
		$week_sales = superman_order_sum('total', array('product_id' => $id, 'more_status' => 0, 'start_time' => $week_start, 'end_time' => $week_end,));
		$data['week_sales'] = $week_sales;
		$month_start = mktime(0, 0, 0, date('m'), 1, date('Y'));
		$month_end = mktime(0, 0, 0, date('m'), date('t'), date('Y'));
		$month_sales = superman_order_sum('total', array('product_id' => $id, 'more_status' => 0, 'start_time' => $month_start, 'end_time' => $month_end,));
...
{
	$all_status = array('-2' => '已删除', '-1' => '已取消', '0' => '待支付', '1' => '待发货', '2' => '已发货', '3' => '已收货', '4' => '已评价',);
...
function superman_order_count_status($uid, $product_id, $status)
{
	...
	$sql = "SELECT COUNT(*) FROM " . tablename('superman_creditmall_order') . " {$where}";
	$params = array(':uid' => $uid, ':product_id' => $product_id, ':status' => $status,);
	return pdo_fetchcolumn($sql, $params);
}
...
{
	$all_status = array('-1' => '失败', '0' => '出价', '1' => '拍中',);
	if ($status !== null && isset($all_status[$status])) {
		return $all_status[$status];
	}
	return $all_status;
...
{
	$all_positions = array('1' => array('id' => 1, 'title' => '首页', 'enable' => 1,), '2' => array('id' => 2, 'title' => '兑换排行', 'enable' => 1,), '4' => array('id' => 4, 'title' => '个人中心', 'enable' => 0,), '5' => array('id' => 5, 'title' => '商品列表', 'enable' => 0,), '6' => array('id' => 6, 'title' => '商品详情', 'enable' => 0,), '7' => array('id' => 7, 'title' => '任务中心', 'enable' => 1,),);
		...
		foreach ($ads as &$item) {
			...
			if ($item['content']) {
				foreach ($item['content'] as $v) {
					$adlist[] = array('title' => $v['title'], 'thumb' => $v['thumb'], 'url' => $v['url'],);
				}
			}
		}
		unset($item);
...
function superman_ad_set(&$item)
{
	...
	$item['ad_time'] = array('start' => $item['start_time'], 'end' => $item['end_time'],);
	$item['position'] = array();
...
function superman_ad_position_fetchall_posid($position_id)
{
	...
	$params = array(':position_id' => $position_id,);
	$sql = "SELECT * FROM " . tablename('superman_creditmall_ad_position') . " {$where}";
	return pdo_fetchall($sql, $params);
}
...
{
	$all_types = array('1' => array('id' => 1, 'title' => '新手任务',), '2' => array('id' => 2, 'title' => '日常任务',), '3' => array('id' => 3, 'title' => '活动任务',),);
	if ($typeid > 0 && isset($all_types[$typeid])) {
		return $all_types[$typeid];
	}
	return $all_types;
...
{
	return array(array('type' => 1, 'name' => 'superman_creditmall_task1', 'title' => '上传头像', 'description' => '个人信息页面上传头像', 'icon' => 'addons/superman_creditmall/template/mobile/images/task1.png', 'url' => './index.php?c=entry&act=setting&do=profile&m=superman_creditmall', 'credit_type' => 'credit1', 'credit_min' => 10, 'credit_max' => 10, 'isshow' => 1, 'builtin' => 1, 'issuperman' => 1, 'allow_repeat' => 0,), array('type' => 1, 'name' => 'superman_creditmall_task2', 'title' => '设置昵称', 'description' => '个人信息页面设置昵称', 'icon' => 'addons/superman_creditmall/template/mobile/images/task2.png', 'url' => './index.php?c=entry&act=setting&do=profile&m=superman_creditmall', 'credit_type' => 'credit1', 'credit_min' => 10, 'credit_max' => 10, 'isshow' => 1, 'builtin' => 1, 'issuperman' => 1, 'allow_repeat' => 0,), array('type' => 1, 'name' => 'superman_creditmall_task3', 'title' => '绑定手机号', 'description' => '个人信息页面设置手机号', 'icon' => 'addons/superman_creditmall/template/mobile/images/task3.png', 'url' => './index.php?c=entry&act=setting&do=profile&m=superman_creditmall', 'credit_type' => 'credit1', 'credit_min' => 10, 'credit_max' => 10, 'isshow' => 1, 'builtin' => 1, 'issuperman' => 1, 'allow_repeat' => 0,), array('type' => 1, 'name' => 'superman_creditmall_task4', 'title' => '绑定邮箱', 'description' => '个人信息页面设置邮箱', 'icon' => 'addons/superman_creditmall/template/mobile/images/task4.png', 'url' => './index.php?c=entry&act=setting&do=profile&m=superman_creditmall', 'credit_type' => 'credit1', 'credit_min' => 10, 'credit_max' => 10, 'isshow' => 1, 'builtin' => 1, 'issuperman' => 1, 'allow_repeat' => 0,), array('type' => 2, 'name' => 'superman_creditmall_task5', 'title' => '分享礼品', 'description' => '分享积分商城中的商品', 'icon' => 'addons/superman_creditmall/template/mobile/images/task5.png', 'url' => './index.php?c=entry&act=share&do=list&m=superman_creditmall', 'credit_type' => 'credit1', 'credit_min' => 1, 'credit_max' => 100, 'isshow' => 1, 'builtin' => 1, 'issuperman' => 1, 'allow_repeat' => 0,), array('type' => 1, 'name' => 'superman_creditmall_task6', 'title' => '关注公众号', 'description' => '微信关注公众号', 'icon' => 'addons/superman_creditmall/template/mobile/images/task6.png', 'url' => '', 'credit_type' => 'credit1', 'credit_min' => 100, 'credit_max' => 100, 'isshow' => 1, 'builtin' => 1, 'issuperman' => 1, 'allow_repeat' => 0,), array('type' => 2, 'name' => 'superman_sign', 'title' => '每日签到', 'description' => '每日签到可获得积分奖励', 'icon' => 'addons/superman_creditmall/template/mobile/images/superman_sign.png', 'url' => './index.php?c=entry&do=sign&m=superman_sign', 'credit_type' => 'credit1', 'credit_min' => 1, 'credit_max' => 5, 'isshow' => 0, 'builtin' => 0, 'issuperman' => 1, 'allow_repeat' => 1,), array('type' => 2, 'name' => 'superman_house', 'title' => '分享楼盘', 'description' => '分享楼盘获得积分奖励', 'icon' => 'addons/superman_creditmall/template/mobile/images/superman_house.png', 'url' => './index.php?c=entry&do=house&m=superman_house', 'credit_type' => 'credit1', 'credit_min' => 1, 'credit_max' => 5, 'isshow' => 0, 'builtin' => 0, 'issuperman' => 1, 'allow_repeat' => 1,), array('type' => 3, 'name' => '', 'title' => '自定义任务', 'description' => '添加自定义活动任务', 'icon' => 'addons/superman_creditmall/template/mobile/images/superman_custom.png', 'url' => '', 'credit_type' => 'credit1', 'credit_min' => 1, 'credit_max' => 5, 'isshow' => 0, 'builtin' => 0, 'issuperman' => 0, 'allow_repeat' => 1,),);
}
...
function superman_task_fetch_name($name, $uniacid = 0)
{
	...
	$params = array(':name' => $name, ':uniacid' => $uniacid,);
	return pdo_fetch($sql, $params);
}
...
function superman_mytask_fetch_uid($uid, $task_id)
{
	...
	$params = array(':uniacid' => $_W['uniacid'], ':uid' => $uid, ':task_id' => $task_id,);
	return pdo_fetch($sql, $params);
}
...
function superman_mc_address_fetch_uid($uid, $isdefault = 1)
{
	...
	$params = array(':uid' => $uid, ':isdefault' => $isdefault,);
	return pdo_fetch($sql, $params);
}
	...
	if ($force_fetch || $result === null) {
		...
		if ($data) {
			...
			if ($credit_type != '' && isset($data[$credit_type])) {
				$result[$credit_type] = array('value' => $data[$credit_type], 'title' => isset($credit_types[$credit_type]['title']) ? $credit_types[$credit_type]['title'] : '',);
				return $result[$credit_type];
			}
			foreach ($data as $k => $v) {
				$result[$k] = array('value' => superman_format_price($v), 'title' => isset($credit_types[$k]['title']) ? $credit_types[$k]['title'] : '',);
			}
		}
	}
	return $result;
