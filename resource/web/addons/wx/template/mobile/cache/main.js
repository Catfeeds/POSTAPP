$(function() {
	...
	var Timer = {
		run: function(starttime) {
			...
			if (starttime > nowtime) {
				...
				dd = parseInt(ts / 60 / 60 / 24, 10);
				hh = parseInt(ts / 60 / 60 % 24, 10);
				mm = parseInt(ts / 60 % 60, 10);
				ss = parseInt(ts % 60, 10);
				dd = this.format(dd);
				hh = this.format(hh);
				mm = this.format(mm);
				ss = this.format(ss);
	...
	$(document).on('pageInit', ".superpage_list", function(e, id, page) {
		...
		function addItems(data, params) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				html += '<div class="col-50 item_wrap">' + '<a href="' + item_url + '">' + '<div class="item_img">' + '    <img src="' + item['cover'] + '" onerror="this.src=\'' + params.img_placeholder + '\'"/>' + '</div>' + '<div class="text-overflow font7">' + item['title'] + '</div>' + '<div class="item_info clearfix font6">';
	...
	$(document).on('pageInit', ".superpage_detail", function(e, id, page) {
		...
		function addItems(data, params) {
			...
			for (var i = 0; i < data.length; i++) {
				item = data[i];
				html += '<li class="item-content">' + '<div class="item-inner font6">' + '<div class="auction_user">' + '<div class="item-media text-left">' + '<img class="avatar" src="' + item['avatar'] + '" onerror="this.src=\'../app/resource/images/heading.jpg\'" alt=""/>' + '<span class="text-overflow">' + item['nickname'] + '</span>' + '</div></div>' + '<div class="auction_credit text-center text-overflow">' + item['credit'] + item['credit_title'] + '</div>' + '<div class="auction_time text-right text-overflow">' + item['dateline'] + '</div>' + '</div></li>';
			}
		...
		$('.btn_checkout', page).click(function() {
			...
			$.ajax({
				...
				success: function(resp) {
					...
					if (resp.errno == 0) {
						setTimeout(function() {
							window.location.href = url + '&act=detail&orderid=' + orderid;
						}, 2000);
					} else {
						t.removeClass('disabled');
					}
	...
	$(document).on('pageInit', ".superpage_profile", function(e, id, page) {
		...
		$('.popup_big_avatar', page).click(function() {
		...
		function addItems(data, credit_title) {
			...
			for (var i = 0; i < data.length; i++) {
				item = data[i];
				html += '<li>' + '<div class="item-content"><div class="item-inner">' + '<div class="item-title-row">' + '<div class="font7">' + item['remark'] + '</div>' + '<div class="credit_time font6">' + item['createtime'] + '</div></div>' + '<div class="credit_num credit_color text-strong">' + item['num'] + '<span class="font6">' + credit_title + '</span></div>' + '</div></div></li>';
			}
	...
	$(document).on('pageInit', ".superpage_mycredit", function(e, id, page) {
		...
		function addItems(data, credit_title) {
			...
			for (var i = 0; i < data.length; i++) {
				item = data[i];
				html += '<li>' + '<div class="item-content">' + '<div class="item-inner">' + '<div class="row no-gutter creditlog_wrap">' + '<div class="col-75">' + '<div class="item-title-row">' + '<div class="font7 text-overflow">' + item['remark'] + '</div>' + '<div class="credit_time font6">' + item['createtime'] + '</div>' + '</div></div>' + '<div class="col-25 text-overflow text-right">' + '<div class="credit_num credit_color text-strong">' + item['num'] + '<span class="font6">' + credit_title + '</span>' + '</div></div></div></div></div></li>'
			}
			$('.log-list ul', page).append(html);
		}
		var loading = false;
		...
		var displayAuction = {
			init: function() {
				var refreshItems = {
					timerId: null,
					init: function() {
						if ($('.auction_display', page).attr('data-auto-refresh') == '1' && $('.btn_bid').length > 0) {
				...
				if ($('.btn_bid', page).length > 0) {
					$('.btn_bid', page).click(function() {
						...
						var text = '<p class="buttons-row">' + '<a href="#" class="button btn_minus_credit"><span class="iconfont">&#xe61a;</span></a>' + '<input type="tel" name="bid_credit" value="' + credit + '"/>' + '<a href="#" class="button btn_plus_credit"><span class="iconfont">&#xe61b;</span></a>&nbsp;' + credit_title + '</p>';
						if (auction_credit > 0) {
