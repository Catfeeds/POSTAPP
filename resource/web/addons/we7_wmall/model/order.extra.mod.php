...
function deliveryer_fetchall($sid = 0, $filter = array()) {
	global $_W;
	$data = pdo_fetchall("SELECT id,sid,deliveryer_id,delivery_type,work_status FROM " . tablename('tiny_wmall_store_deliveryer') . ' WHERE uniacid = :uniacid and agentid = :agentid and sid = :sid and work_status = 1', array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':sid' => $sid), 'deliveryer_id');
	if(!empty($data)) {
