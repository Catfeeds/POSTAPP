...
{
	global $_W;
	$data = pdo_fetchall('SELECT b.* FROM ' . tablename('tiny_wmall_store_clerk') . ' as a left join ' . tablename('tiny_wmall_clerk') . ' as b on a.clerk_id = b.id WHERE a.uniacid = :uniacid AND a.sid = :sid', array(':uniacid' => $_W['uniacid'], ':sid' => $sid));
	return $data;
}
