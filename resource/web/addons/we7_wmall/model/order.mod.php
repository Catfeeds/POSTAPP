...
function order_update_goods_info($order_id, $sid, $cart = array()) {
	...
	$goods_info = pdo_fetchall('SELECT id,cid,title,number,price,total,total_update_type,print_label FROM ' . tablename('tiny_wmall_goods') ." WHERE uniacid = :aid AND sid = :sid AND id IN ($ids_str)", array(':aid' => $_W['uniacid'], ':sid' => $sid), 'id');
		...
		if(!empty($selfDelivery['status'])) {
			...
			if(!empty($discount_temp)) {
				$discount = array(
					...
					'plateform_discount_fee' => 0,
					'agent_discount_fee' => 0,
					'store_discount_fee' => round((10 - $discount_temp['back']) / 10 * $cart['price'], 2),
				);
				$activityed['list']['selfDelivery'] = array('text' => "-￥{$discount['store_discount_fee']}", 'value' => $discount['store_discount_fee'], 'type' => 'discount', 'name' => '自提优惠', 'icon' => 'selfDelivery_b.png', 'store_discount_fee' => $discount['store_discount_fee'], 'agent_discount_fee' => $discount['agent_discount_fee'], 'plateform_discount_fee' => $discount['plateform_discount_fee']);
				$activityed['total'] += $discount['store_discount_fee'];
				$activityed['activity'] += $discount['store_discount_fee'];
...
function order_status_update($id, $type, $extra = array()) {
	...
	if($order['order_plateform'] == 'eleme') {
		...
		mload()->model('plugin');
		pload()->classs('order');
		$elemeOrder = new order($order['sid']);

		$store['openplateform_extra'] = iunserializer($store['openplateform_extra']);
		$config_eleme = $store['openplateform_extra']['eleme'];
		$store['auto_handel_order'] = $config_eleme['order']['auto_handel_order'];
		$store['auto_notice_deliveryer'] = $config_eleme['order']['auto_notice_deliveryer'];
		$store['auto_print'] = $config_eleme['order']['auto_print'];
		$store['delivery_mode'] = $config_eleme['delivery']['delivery_mode'];
	}
	...
	if($type == 'handle') {
		...
		if($order['order_plateform'] == 'eleme') {
			if($extra['role'] != 'eleme') {
				...
				$result = $elemeOrder->confirmOrderLite($order['elemeOrderId']);
				if(is_error($result)) {
					return $result;
				}
			}
		...
		if($order['delivery_type'] == 1 && $order['delivery_status'] >= 4 && empty($extra['force_cancel'])) {
			return error(-1, '该订单由平台配送，配送员已取货，正在配送中， 如需取消订单，请联系平台管理员');
		}
		if(is_open_order($order)) {
			if($extra['role'] != 'eleme') {
				//饿了么开放平台取消订单后,消息推送时不执行此方法
				$result = $elemeOrder->cancelOrderLite($order['elemeOrderId'], $extra['reason'], $extra['remark']);
				if(is_error($result)) {
					return $result;
				}
			}
		...
		if(is_open_order($order) || !$order['is_pay'] || $order['final_fee'] <= 0 || ($order['is_pay'] == 1 && $order['pay_type'] == 'delivery' || $order['pay_type'] == 'cash')) {
			pdo_update('tiny_wmall_order', array('status' => 6, 'delivery_status' => 6), array('uniacid' => $_W['uniacid'], 'id' => $order['id']));
		...
		if(is_open_order($order)) {
			if($extra['role'] != 'eleme') {
				...
				$result = $elemeOrder->receivedOrderLite($order['elemeOrderId']);
				if(is_error($result)) {
					return $result;
				}
			}
		...
		if($order['order_plateform'] == 'eleme') {
			...
			$result = $elemeOrder->replyReminder($remind_log['remindid'], 'custom', $reply);
			if(is_error($result)) {
				return $result;
			}
		} else {
