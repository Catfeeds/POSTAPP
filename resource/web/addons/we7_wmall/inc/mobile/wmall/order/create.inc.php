...
$sid = intval($_GPC['sid']);
$store = store_fetch($sid, array('agentid', 'cid', 'title', 'logo', 'location_x', 'location_y', 'payment', 'invoice_status', 'delivery_type', 'delivery_mode', 'delivery_price', 'delivery_fee_mode', 'delivery_areas', 'delivery_time', 'delivery_free_price', 'pack_price', 'delivery_within_days', 'delivery_reserve_days', 'order_note'));
if(empty($store)) {
...
if($ta == 'submit') {
	...
	$order = array(
		...
		'addtime' => TIMESTAMP,
		'data' => array('cart' => iunserializer($cart['original_data'])),
		'note' => trim($_GPC['note']),
	);
		...
		if(!empty($_W['member']['spread1'])) {
			...
			if(!empty($spreads)) {
				...
				if(!empty($order['spread2'])) {
					...
					$commission_spread2 = round($spread1_rate * $order['final_fee'], 2);
				}
