<?php defined('IN_IA') or exit('Access Denied');?><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('web/common/header', TEMPLATE_INCLUDEPATH)) : (include template('web/common/header', TEMPLATE_INCLUDEPATH));?>
	<ul class="nav nav-tabs">
		<li class="active"><a href="<?php  echo $this->createWebUrl('qr',array('op' => 'list'))?>">管理二维码</a></li>
		<!-- <li><a href="<?php  echo $this->createWebUrl('qr',array('op' => 'add'))?>">生成二维码</a></li> -->
	...
	<div class="panel panel-default">
		...
		<table class="table table-hover">
			...
			<tbody>
				...
				<tr>
					...
					<td>
					<a href="<?php  echo $this->createWebUrl('qr',array('op' => 'add','id' => $row['id']))?>">编辑</a>
					<?php  if($row['model'] == 2) { ?>
						&nbsp;-&nbsp;<a href="<?php  echo $this->createWebUrl('qr',array('op' => 'del','id' => $row['id']))?>" onclick="return confirm('您确定要删除该二维码以及其统计数据吗？')">强制删除</a>
					<?php  } ?>
					<?php  if($row['model'] == 1) { ?>&nbsp;-&nbsp;<a href="<?php  echo $this->createWebUrl('qr',array('op' => 'extend','id' => $row['id']))?>">延时</a><?php  } ?>

					</td>
				</tr>
				<?php  } } ?>
				<?php  if(is_array($dplist)) { foreach($dplist as $item) { ?>
				<tr>
					<td><a href="javascript:void(0);" title="<?php  echo $item['name'];?>"><?php  echo cutstr($item['name'], 8)?></a></td>
					<td><a href="javascript:void(0);" title="<?php  echo $item['keyword'];?>"><?php  echo cutstr($item['keyword'], 8)?></a></td>
					<td><?php  echo $item['modellabel'];?></td>
					<td><?php  echo $item['expire'];?></td>
					<td>
						<?php  if(!empty($item['qrcid'])) { ?>
						<?php  echo $item['qrcid'];?>
						<?php  } else { ?>
						<?php  echo $item['scene_str'];?>
						<?php  } ?>
					</td>
					<td><a href="<?php  echo $item['showurl'];?>" target="_blank">查看</a></td>
					<!--<td><a href="<?php  echo $row['url'];?>" target="_blank" title="<?php  echo $row['url'];?>"><?php  echo $row['url'];?></a></td>-->
					<td style="font-size:12px; color:#666;">
					...
					<td>
						<a href="<?php  echo $this->createWebUrl('qr',array('op' => 'add','id' => $item['id']))?>">编辑</a>
						<?php  if($item['model'] == 2) { ?>
						&nbsp;-&nbsp;<a href="<?php  echo $this->createWebUrl('qr',array('op' => 'del','id' => $item['id']))?>" onclick="return confirm('您确定要删除该二维码以及其统计数据吗？')">强制删除</a>
						<?php  } ?>
						<?php  if($item['model'] == 1) { ?>&nbsp;-&nbsp;<a href="<?php  echo $this->createWebUrl('qr',array('op' => 'extend','id' => $item['id']))?>">延时</a><?php  } ?>
		...
		<table class="table table-hover">
            <tr>
                ...
                <td class="text-left">
                    <a href="<?php  echo $this->createWebUrl('qr',array('op' => 'add','o' => 'region'))?>" class="btn btn-info"><i class='glyphicon glyphicon-plus'></i>生成小区二维码</a>
                    <a href="<?php  echo $this->createWebUrl('qr',array('op' => 'add','o' => 'business'))?>" class="btn btn-success"><i class='glyphicon glyphicon-plus'></i>生成商家二维码</a>
                </td>
            </tr>
        </table>
		</div>
	</div>
	<a href="<?php  echo $this->createWebUrl('qr',array('op' => 'del','id' => $row['id'],'scgq' => 1))?>" onclick="javascript:return confirm('您确定要删除吗？\n将删除所有过期二维码以及其统计数据！！！')" class="btn btn-primary" style="margin-bottom:15px">删除全部已过期二维码</a>
