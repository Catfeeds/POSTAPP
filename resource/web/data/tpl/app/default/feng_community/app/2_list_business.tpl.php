    ...
    <div class="content">
        <div class="list-block media-list" style="margin: 0 0 60px 0">
            <ul>
                ...
                <li>
                    <a href="#" class="item-link item-content" onclick="window.location.href='<?php  echo $this->createMobileUrl('xqsys',array('op' => 'business_detail','id' => $item['id']))?>'">
                        <div class="item-inner">
                            <div class="item-title-row">
                                <div class="item-title"><?php  echo $item['ordersn'];?></div>
                                <div class="item-after"><?php  echo date('Y-m-d H:i', $item['createtime'])?></div>
                            </div>
                            <div class="item-subtitle"><?php  if($item['status']) { ?><span class="label label-success">已支付</span><?php  } else { ?><span class="label label-default">未支付</span><?php  } ?></div>
                            <div class="item-text"><?php  if(empty($item['realname'])) { ?><?php  echo $item['address_realname'];?><?php  } else { ?><?php  echo $item['realname'];?><?php  } ?>-<?php  if(empty($item['mobile'])) { ?><?php  echo $item['address_telephone'];?><?php  } else { ?><?php  echo $item['mobile'];?><?php  } ?>-<?php  echo $item['price'];?>元</div>
                        </div>
                    </a>
                </li>
                <?php  } } ?>
