...
<header class="bar bar-nav bg-green">
    ...
    <h1 class="title txt-fff">我的意见</h1>
</header>
...
<!--<div id="btn_div">-->
    ...
    <!--<div id="menu_b" class="menu_btn hide_m">-->
        ...
        <!--<a href="#" onclick="window.location.href='<?php  echo $this->createMobileUrl('report',array('op' => 'my'))?>'">我的建议</a>-->
    <!--</div>-->
<!--</div>-->
<script type="text/html" id="xq_list">
    ...
    <ul>
        ...
        <li class="item-content">
            <div class="item-inner">
                <div class="item-title-row">
                    <div class="item-subtitle">意见日期：{{ d.list[i].datetime}}</div>
                    ...
                    <div class="item-after"><a class="repair-del" onclick="javascript:window.location.href='<?php  echo $this->createMobileUrl('report',array('op'=> 'rank'))?>&id={{d.list[i].id}}'">待评价</a></div>
                    {{# } }}
                </div>
            </div>
        </li>
...
<script>
    ...
    function del(id){
        var id = id;
        $.post("<?php  echo $this->createMobileUrl('report')?>", {"op":"delete","id":id}, function(msg){
            if (msg.status == 1) {
                setTimeout(function(){
...
<script>
    $(document).ready(function() {
        ...
        loaddata("<?php  echo $this->createMobileUrl('report',array('op' => 'my'))?>&status="+status, $("#data-list-0"),'xq_list', true);
    });
</script>
<script>
    function xqreport(status) {
        var status =status;
        var url = "<?php  echo $this->createMobileUrl('report',array('op'=>'my'))?>&status="+status;
