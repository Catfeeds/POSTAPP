...
if ($operation == "login") {
	...
	$user = pdo_fetch("SELECT * FROM " . tablename('j_money_user') . " WHERE weid='{$_W['uniacid']}' and id=:a and status=1", array(":a" => $deviceinfo));
	if (!$user) {
		die(json_encode(array("success" => false, "msg" => "请先登录", "a" => $_GPC)));
	}
	$shop = pdo_fetch("SELECT * FROM " . tablename('j_money_group') . " WHERE weid='{$_W['uniacid']}' and id=:a ", array(":a" => $user['pcate']));
	...
	if ($result['code'] == "10003") {
		...
		$insertdata = array("transaction_id" => $result['trade_no'], "cash_fee" => intval($result['receipt_amount'] * 100), "time_end" => strtotime($result['gmt_payment']));
	...
	if ($result['code'] == 10003) {
		...
		if ($result['trade_status'] == "TRADE_SUCCESS") {
			$data = array("transaction_id" => $result['trade_no'], "cash_fee" => intval($result['receipt_amount'] * 100), "time_end" => strtotime($result['send_pay_day']));
	...
	$shop = pdo_fetch("SELECT * FROM " . tablename('j_money_group') . " WHERE weid='{$_W['uniacid']}' and id=:a ", array(":a" => $user['pcate']));
	$insert = array("weid" => $_W['uniacid'], "userid" => $_GPC['userid'], "groupid" => $_GPC['groupid'], "out_trade_no" => $_GPC['out_trade_no'], "attach" => "PC", "total_fee" => $_GPC['total_fee'], "order_fee" => $_GPC['order_fee'], "cash_fee" => $_GPC['cash_fee'], "createtime" => $_GPC['createtime'], "status" => $_GPC['status'], "time_end" => $_GPC['time_end'], "isconfirm" => $_GPC['isconfirm'], "memberno" => $_GPC['memberno'], "openid" => $_GPC['openid'], "is_subscribe" => $_GPC['is_subscribe'], "sub_openid" => $_GPC['sub_openid'], "sub_is_subscribe" => $_GPC['sub_is_subscribe'], "trade_type" => $_GPC['trade_type'], "bank_type" => $_GPC['bank_type'], "transaction_id" => $_GPC['transaction_id'], "fee_type" => $_GPC['fee_type'], "createdate" => date("Y-m-d", strtotime($_GPC['createdate'])), "credit" => $_GPC['credit'], "paytype" => $_GPC['paytype']);
	$isHad = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('j_money_trade') . " WHERE weid='{$_W['uniacid']}' and out_trade_no=:a", array(":a" => $_GPC['out_trade_no']));
	if ($isHad) {
	...
	if ($member['cash'] < $fee) {
		die(json_encode(array("success" => false, "msg" => "会员余额不足")));
	}
	...
	$data = array("card_id" => $shop["wxcardid"] ? $shop["wxcardid"] : $cfg["wxcardid"], "code" => $member['wxcardno'], "balance" => intval($member['cash']));
	...
	$shop = pdo_fetch("SELECT * FROM " . tablename('j_money_group') . " WHERE weid='{$_W['uniacid']}' and id=:a ", array(":a" => $user['pcate']));
	$insert = array("weid" => $_W['uniacid'], "userid" => $_GPC['userid'], "groupid" => $_GPC['groupid'], "cardno" => $_GPC['cardno'], "paytype" => $_GPC['paytype'], "cash" => $_GPC['cash'], "out_trade_no" => $_GPC['out_trade_no'], "remark" => $_GPC['remark'], "createtime" => $_GPC['createtime'], "createdate" => date("Y-m-d", strtotime($_GPC['createdate'])), "status" => $_GPC['status']);
	...
	$acc = WeAccount::create($acid);
	$tokens = $acc->fetch_token();
	$data = array("card_id" => $shop["wxcardid"] ? $shop["wxcardid"] : $cfg["wxcardid"], "code" => $member['wxcardno'], "balance" => $cash);
	$pageparama = urldecode(json_encode($data));
	$resp = ihttp_request("https://api.weixin.qq.com/card/membercard/updateuser?access_token=" . $tokens, $pageparama, $xml);
	...
	$pageparama = array("appid" => $payParama['appid'], "mch_id" => $payParama['pay_mchid'], "device_info" => $userOpenid, "nonce_str" => getNonceStr(), "out_trade_no" => $orderid, "out_refund_no" => $refund_trade_no, "total_fee" => $fee, "refund_fee" => $fee, "op_user_id" => $userOpenid, "app_id" => $payParama['app_id'], "alipay_cert" => $payParama['alipay_cert']);
	...
	$url = "";
	$temp = array("first" => array("value" => "【" . $shop['companyname'] . "】【" . $user['realname'] . "】发起退款", "color" => "#FF0000"), "orderProductPrice" => array("value" => sprintf('%.2f', $trade['cash_fee'] / 100), "color" => "#FF0000"), "orderProductName" => array("value" => "电脑端退款成功", "color" => "#333333"), "orderName" => array("value" => $orderid, "color" => "#333333"), "remark" => array("value" => "时间：" . date("Y-m-d H:i:s"), "color" => "#333333"));
	if ($cfg['refunder'] || $shop['refunder']) {
		$acid = pdo_fetchcolumn("SELECT acid FROM " . tablename('account') . " WHERE uniacid=:uniacid ", array(':uniacid' => $_W['uniacid']));
		$acc = WeAccount::create($acid);
		$data = $acc->sendTplNotice($shop['refunder'] ? $shop['refunder'] : $cfg['refunder'], $cfg["tempid2"], $temp, $url, "#FF0000");
	}
	die(json_encode(array("success" => true, "refund_trade_no" => $refund_trade_no, "refundtime" => date("Y-m-d H:i:s"))));
