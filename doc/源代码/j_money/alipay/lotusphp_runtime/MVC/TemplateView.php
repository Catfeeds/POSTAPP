...
class LtTemplateView
{
	...
	protected function parse($str)
	{
		...
		// 回车 换行
		$str = str_replace("{CR}", "<?php  /*捷讯设计*/ echo \"\\r\";?>", $str);
		$str = str_replace("{LF}", "<?php  /*捷讯设计*/ echo \"\\n\";?>", $str); 
		// if else elseif
		$str = preg_replace("/\{if\s+(.+?)\}/", "<?php  /*捷讯设计*/ if(\\1) { ?>", $str);
		$str = preg_replace("/\{else\}/", "<?php  /*捷讯设计*/ } else { ?>", $str);
		$str = preg_replace("/\{elseif\s+(.+?)\}/", "<?php  /*捷讯设计*/ } elseif (\\1) { ?>", $str);
		$str = preg_replace("/\{\/if\}/", "<?php  /*捷讯设计*/ } ?>", $str); 
		// loop
		$str = preg_replace("/\{loop\s+(\S+)\s+(\S+)\}/e", "\$this->addquote('<?php  /*捷讯设计*/ if(isset(\\1) && is_array(\\1)) foreach(\\1 as \\2) { ?>')", $str);
		$str = preg_replace("/\{loop\s+(\S+)\s+(\S+)\s+(\S+)\}/e", "\$this->addquote('<?php  /*捷讯设计*/ if(isset(\\1) && is_array(\\1)) foreach(\\1 as \\2=>\\3) { ?>')", $str);
		$str = preg_replace("/\{\/loop\}/", "<?php  /*捷讯设计*/ } ?>", $str); 
		// url生成
		$str = preg_replace("/\{url\(([^}]+)\)\}/", "<?php  /*捷讯设计*/ echo LtObjectUtil::singleton('LtUrl')->generate(\\1);?>", $str); 

		// 函数
		$str = preg_replace("/\{([a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff:]*\s*\(([^{}]*)\))\}/", "<?php  /*捷讯设计*/ echo \\1;?>", $str);
		$str = preg_replace("/\{\\$([a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff:]*\(([^{}]*)\))\}/", "<?php  /*捷讯设计*/ echo \$\\1;?>", $str); 
		// 变量
		/**
		 * 放弃支持$name.name.name
		 * $str = preg_replace("/\{(\\\$[a-zA-Z0-9_\[\]\'\"\$\x7f-\xff]+)\.([a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)\}/", "<?php  /*捷讯设计*/ echo \\1['\\2'];?>", $str);
		 */
		// 其它变量
		$str = preg_replace("/\{(\\$[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)\}/", "<?php  /*捷讯设计*/ echo \\1;?>", $str);
		$str = preg_replace("/\{(\\$[a-zA-Z0-9_\.\[\]\'\"\$\x7f-\xff]+)\}/e", "\$this->addquote('<?php  /*捷讯设计*/ echo \\1;?>')", $str); 
		// 类->属性  类->方法
		$str = preg_replace("/\{(\\\$[a-zA-Z0-9_\[\]\'\"\$\x7f-\xff][+\-\>\$\'\"\,\[\]\(\)a-zA-Z0-9_\x7f-\xff]+)\}/es", "\$this->addquote('<?php  /*捷讯设计*/ echo \\1;?>')", $str); 
		// 常量
		$str = preg_replace("/\{([A-Z_\x7f-\xff][A-Z0-9_\x7f-\xff]*)\}/", "<?php  /*捷讯设计*/ echo \\1;?>", $str); 
		// 静态变量
		$str = preg_replace("/\{([a-zA-Z0-9_]*::?\\\$[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)\}/", "<?php  /*捷讯设计*/ echo \\1;?>", $str);
		$str = preg_replace("/\{([a-zA-Z0-9_]*::?\\\$[a-zA-Z0-9_\.\[\]\'\"\$\x7f-\xff]+)\}/e", "\$this->addquote('<?php  /*捷讯设计*/ echo \\1;?>')", $str); 
	...
	{
		$str = str_replace(array('<?php  /*捷讯设计*/ exit?>', '<?php  /*捷讯设计*/ exit;?>'), array('', ''), $str); 
		// 删除行首尾空白
		$str = preg_replace("/([\r\n]+)[\t ]+/s", "\\1", $str);
		$str = preg_replace("/[\t ]+([\r\n]+)/s", "\\1", $str); 
		// 删除 {} 前后的 html 注释 <!--  -->
